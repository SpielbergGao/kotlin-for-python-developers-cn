...
没有引用任何内容的变量引用为 `null`（或者你可以说变量 `non-null`）。 与Python中的 `None` 相反，`null` 不是对象-它只是一个关键字，用于使变量不指向任何内容或检查其是否起作用（必须使用 `==` 或 `！=`）。 由于空值是编程错误的常见来源，因此 Kotlin 鼓励尽可能避免它们-除非声明为允许空值，否则变量实际上不能为空值，你可以在类型名后加上 `？`。 例如：
...
编译器将允许将此函数称为例如 `test("a"，"b")` 或 `test("a"，null)`，但不作为 `test(null，"b"）`或 `test(null，null)`。 仅当编译器可以证明 `a` 不可能为空时，才允许调用 `test(a，b)`。 在 `test` 内部，编译器将不允许你对 `b` 进行任何操作，否则如果 `b` 恰好为 null，则会导致异常-因此你可以执行 `a.length`，但不能执行 `b.length`。 但是，一旦你处于检查 `b` 不为 null 的条件中，就可以这样做：
...
`x ?: y` 计算 `x`，除非表达式为 null，否则它将成为表达式的结果，在这种情况下，你将获得 `y`（该值应为不可为 null 的类型）。 这也称为 "Elvis operator"。 你甚至可以使用它来在 null 的情况下提前返回：

```kotlin
val z = x ?: return y
```

如果 `x` 是 non-null，这会将 `z` 分配给 `x` ，但是如果为 null，则包含该表达式的整个函数将停止并返回 `y` （这是有效的，因为 return 也是一个表达式 ，如果对其进行了评估，它将评估其参数，然后使包含函数返回结果）。


### 非空断言操作符

有时，你遇到的情况是你知道一个不为 null 的值 `x` ，但编译器没有意识到。 当你与 Java 代码进行交互时，这可以合法地发生，但是如果发生这种情况是因为代码的逻辑比编译器的推理能力更复杂，则你可能应该重组代码。 如果你不能说服编译器，则可以诉诸 `x!!` 来形成一个表达式，该表达式产生 `x` 的值，但其类型是 non-nullable：
...
当然，它可以作为单个表达式来完成：`val x = javaFunctionThatYouKnowReturnsNonNull（）!!`。

如果值实际上为空，则 `!!` 将引发一个 `NullPointerException`。 因此，如果确实需要调用特定函数，并且如果没有对象可以调用它，则也可能会使用异常（`maybeNull!!.importantFunction()`），尽管有更好的解决方案（因为 NPE 信息不多）是这样的：
...
上面的代码也可能是一个单行代码-并且请注意编译器知道，因为如果 `x` 为 null，则 `throw` 将阻止 `y` 的出现，因此，如果我们到达下面的行，则 `y` 必须为非 null 。 将此与 `x?.importantFunction()` 相比，如果 `x` 为 null 则为空操作。
